...
def getConstantTensor(value, shape=None, dtype=None, device=None):
...
def matrix(*rows, device=None):
    ...
    if len(ref) == 0:
        return getConstantTensor(np.asarray(rows), device=device)
    assert device is None or device == ref[0].device
    elems = [x if isinstance(x, torch.Tensor) else getConstantTensor(x, shape=ref[0].shape, device=ref[0].device) for x in elems]
...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if G_inv is not I_3:
            ...
            margin = margin + getConstantTensor([Hz_pad * 2 - cx, Hz_pad * 2 - cy] * 2, device=device)
            margin = margin.max(getConstantTensor([0, 0] * 2, device=device))
            margin = margin.min(getConstantTensor([width-1, height-1] * 2, device=device))
        ...
        v = getConstantTensor(np.asarray([1, 1, 1, 0]) / np.sqrt(3), device=device) # Luma axis.
        if self.lumaflip > 0:
        ...
        if self.imgfilter > 0:
            ...
            expected_power = getConstantTensor(np.array([10, 1, 1, 1]) / 13, device=device) # Expected power spectrum (1/f).
